...
class content_encoder(nn.Module):
    ...
    def __init__(self, G_ch=64, G_wide=True, resolution=128,
        ...
        for index in range(len(self.arch['out_channels'])):
            if index == 0:
                self.blocks += [[DBlock(in_channels=1,
                ...
                # self.block = DBlock(in_channels=1,
                #                     out_channels=self.arch['out_channels'][index],
                #                     which_conv=self.which_conv,
                #                     wide=self.G_wide,
                #                     activation=self.activation,
...
class decoder_textedit_addskip(nn.Module):
    ...
    def forward(self, x, residual_features, style_emd = None, style_fc = None,residual_features_style = None):
        #import pdb;pdb.set_trace()
        adapt_params = self.MLP(style_fc)  # （16，9984）
        ...
        outp = self.blocks[-1](h)
        outp = torch.tanh(outp)
        out = torch.mean(outp,axis=1,keepdim=True)
...
class style_encoder_textedit_addskip(nn.Module):
    ...
    def __init__(self, G_ch=64, G_wide=True, resolution=128,
        ...
        for index in range(len(self.arch['out_channels'])):
            if index == 0:
                self.blocks += [[DBlock(in_channels=6,
                                        out_channels=self.arch['out_channels'][index],
                                        which_conv=self.which_conv,
                                        wide=self.G_wide,
                                        activation=self.activation,
